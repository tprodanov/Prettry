...
void trim(int argc, char const** argv) {
    ...
    std::string usage =
    R"(Prettry by Timofey Prodanov (timofey.prodanov@gmail.com)
Usage:
    )" + name + R"( [options] -a <ampl.fa> -f [<in.fq>] [-o <out.fq>] [-e <err.fq>]
    )" + name + R"( [options] -a <ampl.fa> -f <in1.fq> <in2.fq> -o <out1.fq> <out2.fq> [-e <err1.fq> <err2.fq>]
    )" + name + R"( [options] -a <ampl.fa> -s [<in.sam>] [-o <out.sam>] [-e <err.sam>]
...
Other:
    ...
    atlas::parse_args parser(usage, "1.0");
